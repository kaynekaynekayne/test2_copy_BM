  ...
  <div class="container p-0">
    ...
    <div class="modalContent mt-3 pb-3">
      ...
      <div class="text-right mt-4 pr-5">
        <b-button id="btnCancelAccount" class="custom-gray-btn" @click="onClose" size="sm">Cancel</b-button>
        <b-button id="btnCreateAccount" class="custom-blue-btn" @click="createAccount" size="sm">OK</b-button>
      </div>
    </div>
  </div>
</template>
      ...
      createAccount (evt) {
        ...
        if (!this.inputsState.newUserIdState || !this.inputsState.passwordState ||
          ...
          var userData = {
            ...
            createDttm: this.$getDateTime(),
            modifyDttm: this.$getDateTime(),
            loginDttm: this.$getDateTime(),
            isAvailable: 'Y'
          }
          ...
          // this.$store.dispatch('addUser', userData).then(function(isSignup) {
          //   if (isSignup) {
          //     self.$emit('close')
          //   } else {
          //     self.$toasted.show(Constant.IDS_ERROR_SIGINUP_ALREADY_ID, {
