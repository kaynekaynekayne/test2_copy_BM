...
CommonUtils.install = function (Vue, options) {
  ...
  Vue.prototype.$getOrderNo = function () {
    ...
    var orderNo = 'ORD' + year + month + day + hour + minute + second + milliSecond
    for (var i = 0; i < 5; i++) {
      orderNo = orderNo.concat(Math.floor(Math.random() * 9) + 1)
    }
    return orderNo
  }
  Vue.prototype.$randomNum = function(n1, n2) {
  ...
  // 만나이 계산
  Vue.prototype.$calcAge = function (birth) {
    var date = new Date()
  ...
  Vue.prototype.$tileSourceFromData = function(path) {
  ...
  Vue.prototype.$checkBmNormalCell = function(bmInfo) {
    var isNormal = 'Y'

    // bmInfo.forEach(function(bmItem) {
    //
    // })
  ...
  Vue.prototype.$checkPbNormalCell = function(wbcInfo) {
    var norMalRange = this.$store.getters.getNormalRange
    var totalCount = 0
  ...
  // print report
  Vue.prototype.$makePrintHtml = function (resultItem, wbcList, rbcList, totalCount) {
    ...
    var html =    `<span>&#60; Hospital &#62; &nbsp;&nbsp;</span>` +
                  ...
                  `<div class="userContainer" style="display: flex; flex-wrap: wrap; margin-top: 40px; font-size: 15px;">` +
                    ...
                    `</div>` +
                    `<div class="item" style="flex-basis: 50%;">` +
                      resultItem.GENDER_NM +
                    `</div><br /><br />`
                    if (resultItem.ANALYSIS_TYPE === '01') {
                      ...
                      html += `<div class="item" style="flex-basis: 50%;">` +
                                createBmClassification(resultItem, wbcList, totalCount) +
                              `</div>` +
                              `<div class="item">` +
                                drawBmPrintImages(resultItem, wbcList, this.$store.getters.getSettings) +
                              `</div>`
                    }
                   + `</div>`
    return html
  ...
  var drawWbcPrintImages = function (resultItem, wbcList, settings) {
    if (settings.modelWbcReportPrint.length > 0) {
      ...
      var classDir = rootPath + '/' + settings.wbcClassDirName
  ...
  var drawBmPrintImages = function (resultItem, wbcList, settings) {
    console.log(settings.modelBmReportPrint)
    if (settings.modelBmReportPrint.length > 0) {
      var rootPath = settings.pbiaRootPath + '/' + resultItem.SLOT_ID
      var classDir = rootPath + '/' + settings.wbcClassDirName
      ...
      var result = '<div class="wbcImagePrintContainer" style="display: flex; flex-wrap: wrap;">'
      for (var i = 0; i < wbcList.length; i++) {
        for (var j = 0; j < settings.modelBmReportPrint.length; j++) {
          if (wbcList[i].id === settings.modelBmReportPrint[j]) {
            if (wbcList[i].images.length > 0) {
              result += `<div style="flex-basis: 100%; margin-top: 10px;">` +
                          `<div style="width: 180px; border-bottom: 2px solid #09CBFC;">` +
                            wbcList[i].name +
  ...
  // Vue.prototype.$ipcRendererSend = function(parent, params) {
  //   console.log(params)
  //   return new Promise(function(resolve, reject) {
  //     ipcRenderer.send('SEND_DATA', JSON.stringify(params))
  //     ipcRenderer.once('RECEIVE_DATA', function (evt, resultObj) {
  //       console.log(resultObj)
  //       if (resultObj.resultCd === '00000') {
  //         resolve(resultObj)
  //       } else {
  //         reject(new Error('[CMD ' + resultObj.jobCmd + ']' + '[' + resultObj.resultCd + ']' + resultObj.resultMsg))
  //       }
  //     })
  //   })
  // }
  // Vue.prototype.$ipcRendererSendSysInfo = function(parent, params) {
  //   console.log(params)
  //   return new Promise(function(resolve, reject) {
  //     ipcRenderer.send('SEND_DATA', JSON.stringify(params))
  //   })
  ...
  var createBmClassification = function (resultItem, bmList, totalCount) {
    var result = '<div class="bmContainer" style="display: flex; flex-wrap: wrap;">'
    result += `<div class="item" style="flex-basis: 100%;">BM Cell Diff count classification` + `</div><br /><br />`
    ...
    for (var i = 0; i < bmList.length; i++) {
      if ((bmList[i].title !== 'OT' &&
           bmList[i].title !== 'MC')) {
        if (bmList[i].title === 'BL' || bmList[i].title === 'PR' || bmList[i].title === 'MY' ||
            bmList[i].title === 'ME' || bmList[i].title === 'NS' || bmList[i].title === 'NB' ||
            bmList[i].title === 'EO' || bmList[i].title === 'BA') {
          mTotal += bmList[i].count
        } else if (bmList[i].title === 'PE' || bmList[i].title === 'BN' || bmList[i].title === 'PN' ||
                   bmList[i].title === 'ON') {
          eTotal += bmList[i].count
        }
        result += `<div class="item" style="flex-basis: 40%;">` + bmList[i].name + `</div>`
        result += `<div class="item" style="flex-basis: 30%; text-align: right;">` + bmList[i].count + `</div>`
        result += `<div class="item" style="flex-basis: 30%; text-align: right;">` + bmList[i].percent + `%</div>`
      }
    ...
    for (var j = 0; j < bmList.length; j++) {
      if ((bmList[j].title === 'OT' ||
           bmList[j].title === 'MC')) {
        result += `<div class="item" style="flex-basis: 40%;">` + bmList[j].name + `</div>`
        result += `<div class="item" style="flex-basis: 30%; text-align: right;">` + bmList[j].count + `</div>`
        result += `<div class="item" style="flex-basis: 30%; text-align: right;">` + `</div>`
      }
    }

    meRatio = (mTotal / eTotal).toFixed(2)
    result += `<div class="item" style="flex-basis: 40%;">M:E Ratio</div>`
    result += `<div class="item" style="flex-basis: 30%; text-align: right;">` + meRatio + ` : 1</div>`
    result += `<div class="item" style="flex-basis: 30%; text-align: right;">` + `</div>`

    result += `<br /><br /><div class="item" style="flex-basis: 100%;">` + 'Comment' + `</div>`
    result += `<div class="item" style="flex-basis: 100%; word-break: break-all;">>&nbsp` + resultItem.BM_COMMENT + `</div>`
